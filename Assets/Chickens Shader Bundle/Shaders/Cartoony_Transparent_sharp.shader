Shader "Chickenlord/Toony/Transparent/Lighted Outline Specularmap Sharp" {
	Properties {
	...
	SubShader {
		...
		CGPROGRAM
		#pragma surface surf ToonyBlinnPhong alpha
		#pragma target 3.0
		...
		inline fixed4 LightingToonyBlinnPhong_PrePass (SurfaceOutputPS s, half4 light)
		{
			...
			half stx = _Steps/256;
			spec = ((int)((spec*256+_Steps/2)/_Steps))*stx;
			fixed diff = Luminance(light.rgb);
			diff = clamp(((int)((diff*256+_Steps/2)/(_Steps)))*(stx),0,1);
