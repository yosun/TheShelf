Shader "Chickenlord/Mobile/Free/Vertex Lit + Indirect Specular" {
	Properties {
	...
	SubShader {
		...
		void vert (inout appdata_full v, out Input o)
		{
			...
			fixed3 shv = CSBShadeSH9 (float4(worldN,1.0))+UNITY_LIGHTMODEL_AMBIENT.rgb;
			o.shV = shv;
			fixed3 viewDir = (WorldSpaceViewDir(v.vertex));
			float3 reflected = normalize(reflect(-viewDir,worldN));
			o.shVs = pow(saturate(dot((reflected),worldN)),_SpecPow*128)*(CSBShadeSH9(float4(reflected,1.0))+UNITY_LIGHTMODEL_AMBIENT.rgb);
			#ifndef VERTEXLIGHT_ON
			o.shV += saturate(dot(worldN,_WorldSpaceLightPos0.xyz));
			o.shVs += pow(saturate(dot(normalize(normalize(viewDir)+_WorldSpaceLightPos0.xyz),worldN)),_SpecPow*128);
			#endif
		}
