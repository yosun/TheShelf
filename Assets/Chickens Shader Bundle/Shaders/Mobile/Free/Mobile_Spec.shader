Shader "Chickenlord/Mobile/Free/Mod/Bumped Specular + Indirect Specular" {
Properties {
	...
	_ISI("Indirect Specular Intensity",Float) = 3
...
inline fixed4 LightingBlinnPhongPS (SurfaceOutputPS s, fixed3 lightDir, fixed atten)
{
	...
	fixed4 c;
	c.rgb = _GM*s.Albedo*(spec*s.Gloss*2.6f+s.DiffuseProbes*(dl*_KColor+max(0,_FillAmmount-dl-bl)*_FillColor+bl*_BackColor))+s.iSpec*is*s.Gloss;
	c.a = 0;
	return c;
}
...
v2f_surf vert_surf (appdata_full v) {
  ...
  o.worldRefl = CSBShadeSH9(float4(normalize(reflect(worldRefl,worldN)),1))*_ISI;
  ...
  o.probes = CSBShadeSH9(float4(worldN,1))*2;
