Shader "Chickenlord/Mobile/Free/Skin" {
	Properties {
	...
	SubShader {
		...
		void vert (inout appdata_full v, out Input o)
		{
			...
			o.sh = CSBShadeSH9(float4(worldN,1))+UNITY_LIGHTMODEL_AMBIENT.rgb;
			fixed3 viewDir = normalize(WorldSpaceViewDir(v.vertex));
			o.skySpec.x = (pow(saturate(dot(normalize(viewDir+WorldSpaceLightDir(v.vertex)),worldN)),_Shininess*128));
		...
		inline fixed4 LightingST (SurfaceOutputPS s, fixed3 lightDir, fixed atten)
		{
			...
			fixed4 c;
			c.rgb = ((s.Albedo*(colv+s.sh)) + s.SkySpec)*_GM;
			c.a = s.Alpha;
			return c;
		}
