Shader "Chickenlord/3.5/Skin/Simple SSS Skinshader" {
	Properties {
	...
	SubShader {
		...
		inline fixed4 LightingBlinnPhongSSSLayer3 (SurfaceOutputPS s, fixed3 lightDir, fixed3 viewDir, fixed atten)
		{
			...
			c3.rgb = (s.Albedo * (lightColor) * diff +_LightColor0.rgb* _SpecColor.rgb * spec) * (atten * 2);
			c3.a = 1;
			...
			c2.rgb = (s.Albedo * (lightColor) * diff + _LightColor0.rgb* _SpecColor.rgb * spec) * (atten * 2);
